...
5. Provider 只解析有界 UTF-8 输出中的完整稳定版 libtest `test result:` 计数，并聚合多个 test binary。状态区分 passed、failed、crashed、timed_out、no_tests、provider_failed；coverage 区分 covered、empty、unknown。
...
- Python Test Provider 仍是下一项未完成的跨语言 Test 合同。
